CREATE OR REPLACE Function COTAFA.ufTBL_4230 (
  SP_COM_ID              IN VARCHAR2,
...
--------------------------------------------------------------------------------
  ...
  deliKind            VARCHAR2(100 Char),
  deliStatus          VARCHAR2(100 Char),
  rcptKoPapay         NUMBER
  );  
...
BEGIN
  ...
  IF SP_Kind = 'DATA' THEN
    ...
    vQuery := vQuery || '       ufName('''||SP_Com_ID||''', ''ORD_MST.STATUS'', A.Status) AS Status';                             
    ...
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''CENTER'',  A.Cnt_CD) AS cntNameOrd   ';                             
    ...
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''CODE'',  B.User_Kind) AS userKindName';                             
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''CENTER'',B.Cnt_CD)    AS cntNameMem  ';                             
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''GROUP'', B.Grp_CD)    AS grpName     ';                             
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''RANK'',  B.Rank_CD)   AS RankName    ';                             
    vQuery := vQuery || '     , B.R_ID AS rId                   ';                                                                
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''USERNAME'',B.R_ID) || ''('' || B.R_ID|| '')'' AS rName  ';          
    vQuery := vQuery || '     , A.Omni_YN AS omniYN             ';                                                                
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''COUNTRY'', A.CTR_CD) AS countryName  ';                             
    vQuery := vQuery || '     , A.Kind_CD AS kindCd             ';                                                                
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''CODE'',    A.Kind_CD) AS kindName    ';                             
    vQuery := vQuery || '     , A.Path_CD AS pathCd             ';                                                                
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''CODE'',    A.Path_CD) AS pathName    ';                             
    vQuery := vQuery || '     , A.Sum_Pdt_Cnt || ''/'' || A.Sum_Pdt_Qty AS sumPdt          ';                                     
    vQuery := vQuery || '     , (Select Count(1) From Ord_Pdt Where Com_Id = ''' || SP_Com_ID || ''' And Ord_No = A.Ord_No) AS sumPdtCnt      ';                                                                
    vQuery := vQuery || '     , (Select Sum(QTY) From Ord_Pdt Where Com_Id = ''' || SP_Com_ID || ''' And Ord_No = A.Ord_No) AS sumPdtQty      ';                                                                
    ...
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''ORD_MST.RCPT_YN'', A.Rcpt_YN) AS rcptYN ';                          
    ...
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''ORD_DELI.DELI_KIND'', A.Ord_No) AS deliKind ';
    vQuery := vQuery || '     , ufName('''||SP_Com_ID||''', ''ORD_PDT.DELI_FINISH'', A.Ord_No) AS deliStatus ';
    vQuery := vQuery || '     , A.Rcpt_KoPapay AS rcptKoPapay   ';
  ELSIF SP_Kind = 'COUNT' THEN                                      
    vQuery :=           'SELECT COUNT(1) AS vCount  ';
  ELSIF SP_Kind = 'SUM' THEN                                      
    vQuery :=           'SELECT ';
    vQuery := vQuery || '       SUM((Select Count(1) From Ord_Pdt Where Com_Id = ''' || SP_Com_ID || ''' And Ord_No = A.Ord_No)) AS sumPdtCnt      ';                                                                
    vQuery := vQuery || '     , SUM((Select Sum(QTY) From Ord_Pdt Where Com_Id = ''' || SP_Com_ID || ''' And Ord_No = A.Ord_No)) AS sumPdtQty      ';                                                                
    ...
    vQuery := vQuery || '     , SUM(A.Rcpt_KoPapay)  AS rcptKoPapay  ';
  END IF;
  
  vQuery := vQuery || '  FROM Ord_Mst A           ';
  vQuery := vQuery || '     , Member B            ';
  vQuery := vQuery || '     , Ord_Deli C            ';
  vQuery := vQuery || '     , (select min(deli_seq) AS Deli_Seq, ord_no FROM Ord_Deli GROUP BY Ord_NO) E ';
  vQuery := vQuery || ' WHERE A.Userid = B.Userid ';
  vQuery := vQuery || '   AND A.Ord_NO = C.Ord_NO ';
  vQuery := vQuery || '   AND A.Ord_NO = E.Ord_NO ';
  vQuery := vQuery || '   AND C.Deli_Seq = E.Deli_Seq ';
  vQuery := vQuery || '   AND A.Com_ID = ''' || SP_Com_ID || ''' ';
  vQuery := vQuery || '   AND C.DELI_SEQ = 1 ';
  ...
  IF (NVL(Length(LTrim(SP_Userid)), 0)     <> 0) THEN                                                                            
    ...
    IF SP_ChkUserid = '3' THEN vQuery := vQuery || ' AND A.Userid IN (SELECT Userid FROM Member WHERE Com_Id =''' || SP_Com_Id || ''' START WITH Userid =''' || SP_Userid || ''' CONNECT BY P_ID = PRIOR Userid) '; END IF;
    IF SP_ChkUserid = '4' THEN vQuery := vQuery || ' AND A.Userid IN (SELECT Userid FROM Member WHERE Com_Id =''' || SP_Com_Id || ''' START WITH Userid =''' || SP_Userid || ''' CONNECT BY Userid = PRIOR P_ID) '; END IF;
    IF SP_ChkUserid = '5' THEN vQuery := vQuery || ' AND B.R_ID   = ''' || SP_Userid     || ''' '; END IF;
    IF SP_ChkUserid = '6' THEN vQuery := vQuery || ' AND A.Userid IN (SELECT Userid FROM Member WHERE Com_Id =''' || SP_Com_Id || ''' START WITH Userid =''' || SP_Userid || ''' CONNECT BY R_ID = PRIOR Userid) '; END IF;
    IF SP_ChkUserid = '7' THEN vQuery := vQuery || ' AND A.Userid IN (SELECT Userid FROM Member WHERE Com_Id =''' || SP_Com_Id || ''' START WITH Userid =''' || SP_Userid || ''' CONNECT BY Userid = PRIOR R_ID) '; END IF;
  END IF;
  ...
  IF (NVL(Length(LTrim(SP_Addition)), 0) <> 0) THEN 
    ...
    IF SP_ChkAddition = '8' THEN vQuery := vQuery || ' AND A.License_No LIKE ''%' || SP_Addition || '%'' '; END IF; --Added by Tai
  END IF;
